...
int search_index(int argc, char** argv) {
  ...
  std::string hnsw_prefix(argv[ctr++]);
  _u32        num_levels = std::atoi(argv[ctr++]);
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int64_t i = 0; i < (int64_t) query_num; i++) {
      ...
      hnsw.search(query + i * query_aligned_dim, recall_at, search_params,
  ...
  if (argc < 11) {
    std::cout
        ...
        << "  [data_type<float/int8/uint8>]  [dist_fn (l2/mips/fast_l2)] "
           "[hnsw_prefix] [num_levels]  [num_threads] "
